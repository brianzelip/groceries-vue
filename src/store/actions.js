...
export const addItemTousiArr = ({ commit }, payload) => {
  //payload is an item object
  commit('ADD_ITEM_TO_USI_ARR', payload);
};
...
export const updateItemInusiArr = ({ commit }, payload) => {
  //payload is an object like { _id, key, value }
  commit('UPDATE_ITEM_IN_USI_ARR', payload);
};

export const resetUserSelectedItems = ({ commit }) => {
  commit('RESET_USER_SELECTED_ITEMS');
};
export const resetusiArr = ({ commit }) => {
  commit('RESET_USI_ARR');
};
...
export const removeItemFromusiArr = ({ commit }, payload) => {
  //payload is an item _id string
  commit('REMOVE_ITEM_FROM_USI_ARR', payload);
};
